								...
								<header>
									<div class="row">
										...
										<div class="col company-details">
											...
											<div>{{ $page.props.app_addresss_invoice }}</div>
											<div>{{ $page.props.app_whatsapp_invoice }}</div>
										</div>
									</div>
								</header>
								<main>
									<div class="row contacts">
										<div class="col invoice-to">
											...
											<div class="address">
                                                {{ transaction.user.student.address }},
                                                Desa/Kel. {{ transaction.user.student.village.name }},
                                                Kec. {{ transaction.user.student.district.name }},
                                                Kota/Kab. {{ transaction.user.student.city.name }},
                                                Provinsi {{ transaction.user.student.province.name }}
                                                <br>
									...
									<table>
										...
										<tbody>
											<tr>
												<td class="no">01</td>
												<td class="text-left">
													Pembelian {{ transaction.exam.title }}
                                                </td>
												<td class="unit">{{ formatPrice(transaction.total_purchases)  }}</td>
												<td class="qty">{{ formatPrice(0)  }}</td>
												<td class="total">{{ formatPrice(transaction.total_purchases) }}</td>
											</tr>
										</tbody>
										<tfoot>
											<tr>
                                    ...
                                    <div class="row row-cols-1 row-cols-md-2 row-cols-lg-3 row-cols-xl-3">
                                        <div class="col" v-for="bank in banks">
                                            <div class="card border-bottom border-3 border-0">
                                                <div style="height:75px;" class="mb-3">
                                                    <center>
									...
									<div class="notices">
										...
										<div class="notice">
                                            ...
                                            Setelah melakukan pembayaran, silakan lakukan konfirmasi melalui Whatsapp di <b>081212126043</b> dengan menyertakan nomor faktur serta bukti transfer pembayaran. terimakasih
                                        </div>
									</div>
                                    <h5>
                                    </h5>
